...
func TestGetUpdatePriority(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			vpa:  test.VerticalAutoscaler().WithContainer(containerName).WithTarget("2", "").Get(),
			expectedPrio: PodPriority{
				OutsideRecommendedRange: false,
				ResourceDiff:            0.5,
				ScaleUp:                 false,
			...
			vpa:  test.VerticalAutoscaler().WithContainer(containerName).WithTarget("900m", "").Get(),
			expectedPrio: PodPriority{
				OutsideRecommendedRange: false,
				ResourceDiff:            89.0,
				ScaleUp:                 true,
			...
			vpa:  test.VerticalAutoscaler().WithContainer(containerName).WithTarget("6", "20M").Get(),
			expectedPrio: PodPriority{
				OutsideRecommendedRange: false,
			...
			vpa:  test.VerticalAutoscaler().WithContainer(containerName).WithTarget("2", "20M").Get(),
			expectedPrio: PodPriority{
				OutsideRecommendedRange: false,
			...
			vpa:  test.VerticalAutoscaler().WithContainer(containerName).WithTarget("2", "10M").Get(),
			expectedPrio: PodPriority{
				OutsideRecommendedRange: false,
...
func TestGetUpdatePriority_NoRecommendationForContainer(t *testing.T) {
	...
	vpa := test.VerticalAutoscaler().WithName("test-vpa").WithContainer("test-container").Get()
